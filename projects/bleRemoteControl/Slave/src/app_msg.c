...
//            if (++g_no_action_cnt > G_NO_ACTION_CNT)
            {
                if (app_state_get() < APP_READY)
                {
                    g_no_action_cnt = 0;
                    keys_sleep();
//                    ke_timer_set(APP_TIMER_RC32K_CORR, TASK_APP, 0xFFFFFFF);
                }
            }
        } break;
        #endif
        ...
        case APP_TIMER_ADV_LED:
        {
            ...
            {
                if (ke_timer_active(APP_TIMER_ADV_LED, TASK_APP))
                {
                    GPIO_DIR_SET_LO(ADV_LED_GPIO);
                    ke_timer_clear(APP_TIMER_ADV_LED, TASK_APP);
                }
            }
        } break;
