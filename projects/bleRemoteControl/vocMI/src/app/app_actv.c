...
// 广播超时处理
static void pt_adv_timeout_proc(uint8_t status)
{
    if (status == GAP_ERR_TIMEOUT)
    {
        PT_LOGD("adv %d timeout\n", actv_env.advtype);
...
static void app_adv_create(void)
{
    ...
    adv_param.disc_mode             = pt_adv_params[actv_env.advtype].disc_mode;
    // Advertising properties (@see enum gapm_adv_prop)
    adv_param.prop                  = pt_adv_params[actv_env.advtype].prop;
    ...
    adv_param.prim_cfg.adv_intv_min = pt_adv_params[actv_env.advtype].interval;
    adv_param.prim_cfg.adv_intv_max = pt_adv_params[actv_env.advtype].interval;

    const uint8_t *peer_addr = pt_read_peer_addr();
    if (actv_env.advtype == ADV_POWER)
    {
        app_adv_data_power[28] = peer_addr[0];
        app_adv_data_power[29] = peer_addr[1];
        app_adv_data_power[30] = peer_addr[2];
        PT_LOGD("adv_data_power %X %X %X\n", app_adv_data_power[28], app_adv_data_power[29], app_adv_data_power[30]);
    }
...
static void app_adv_set_adv_data(void)
{
    gapm_set_adv_data(actv_env.advidx, GAPM_SET_ADV_DATA, 
                    pt_adv_params[actv_env.advtype].adv_data_size, pt_adv_params[actv_env.advtype].adv_data);
}
...
void app_adv_action(uint8_t actv_op)
{
    switch (actv_op)
    {
        ...
        case ACTV_START:
        {
            ...
            {
                DEBUG("Starting");
                gapm_start_advertising(actv_env.advidx, pt_adv_params[actv_env.advtype].timeout);
...
void pt_set_adv_type(pt_adv_type_t advtype)
{
    // /********************************************************/
    // // Add. 2024.05.22 --- wq.
    ...
    PT_LOGD("pt_set_adv_type %d\n", advtype);
    actv_env.advtype = advtype;
}

pt_adv_type_t pt_get_adv_type(void)
{
