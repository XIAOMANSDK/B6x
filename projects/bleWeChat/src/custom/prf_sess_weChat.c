...
/// Serial Service UUID
#define SES_SVC_UUID                ATT_UUID16(0xFEE7)
/// Serial Write Command UUID
#define SES_CHAR_RXD_WRITE          ATT_UUID16(0xFEC7)
/// Serial Notify UUID
#define SES_CHAR_TXD_NOTIFY         ATT_UUID16(0xFEC8)
...
uint8_t sess_ind_send(uint8_t conidx, uint16_t len, const uint8_t* data)
{
    ...
    if ((len > 0) && (sess_env.nb_pkt > 0))
    {
        ...
        {
            status = LE_SUCCESS;
            gatt_ind_send(conidx, sess_get_att_handle(SES_IDX_TXD_VAL), len, data);
...
//    debugHex(data, len);
    ...
    {
        uint16_t lenEnd = RXD_BUFF_SIZE - uart1_rxd.head;
        memcpy(&uart1_rxd.data[uart1_rxd.head], data, lenEnd);
        memcpy(&uart1_rxd.data[0], &data[lenEnd], len - lenEnd);
    }

    uart1_rxd.head = (uart1_rxd.head +len) % RXD_BUFF_SIZE;
