...
// Mark DMAC_IRQHandler as weak so it can be overridden by unified handler in dma_handler.c
// This resolves "multiply defined" linker error when both uart1Rb.c and speaker.c
// define DMAC_IRQHandler. The unified handler in dma_handler.c provides
// strong (non-weak) definition that overrides this weak one.
//
// DMA channels handled:
// - DMA_CH7: UART1 RX (handled by uart1_dma_rx_done())
// - DMA_CH0: Speaker PWM (handled by speaker_dma_handler() in dma_handler.c)
