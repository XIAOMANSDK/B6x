...
#if (GSM_DECODE_EN)
    [CMD_GSMPLAY]   = {11, 28, "AT+GSMPLAY"       },
    [CMD_GSMSTOP]   = {10, 10, "AT+GSMSTOP\r\n"    },
    [CMD_GSMSTATUS] = {13, 13, "AT+GSMSTATUS?\r\n" },
#endif
};
...
// flag: All_FACTORY_REST:  È«ï¿½ï¿½ï¿½Ö¸ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
// flag: PAIR_FACTORY_RESET: ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ï¢
...
//    {
//        //sprintf((char*)sys_config.pass, "000000");      //ï¿½ï¿½ï¿½ï¿½
...
//            } break;
        ...
        case CMD_CON_MAC_R:
        {
            ...
            struct gap_bdaddr* peer_bdaddr = gapc_get_bdaddr(0, GAPC_SMP_INFO_PEER);
...
//            atSetBleDefault(All_FACTORY_REST);
        ...
        {
            DEBUG("[AT]OK\r\n");
            g_cfg_change = CFG_CHNG_RST;// Ö±½ÓÖØÆô¼´¿É
        } break;
        ...
        case CMD_GSMPLAY:
        {
            uint32_t addr = GSM_FLASH_ADDR;  // Default address

            // Parse address if provided (AT+GSMPLAY=<addr>)
            if (buff_len > 11 && buff[10] == '=')
            {
                // Parse hex address from string (AT+GSMPLAY=0x18010000\r\n)
                char addr_str[12];
                uint8_t addr_len = buff_len - 13;  // Remove "AT+GSMPLAY=" and "\r\n"

                if (addr_len > 0 && addr_len <= 10)
                {
                    memcpy(addr_str, &buff[11], addr_len);
                    addr_str[addr_len] = '\0';
                    addr = strtoul(addr_str, NULL, 16);
                }
            }

            //int frames = gsm_play_from_flash(addr, 0);  // Play once (not loop)
            int frames = gsm_play_start_memory(addr, 1, 0); // need to fix
            if (frames >= 0)
            {
                DEBUG("[AT]OK\r\n");
                DEBUG("[DA]Playing %d frames from 0x%08X\r\n", frames, addr);
            }
            else
            {
                DEBUG("[AT]ERR[%d]\r\n", ERR_OPERATION);
        ...
        case CMD_GSMSTOP:
        {
            gsm_audio_stop();
            DEBUG("[AT]OK\r\n");
            DEBUG("[DA]Playback stopped\r\n");
        } break;

        case CMD_GSMSTATUS:
        {
            uint8_t playing = gsm_audio_is_playing();
            DEBUG("[AT]OK\r\n");
            DEBUG("[DA]GSM Status=%s\r\n", playing ? "Playing" : "Stopped");
        } break;
#endif
...
//    DEBUG("cfg:%X\r\n", g_cfg_change);
    ...
    {
        #if ((LED_PLAY) || (CFG_SFT_TMR))
        sftmr_wait(6); // ÉèÖÃ²ÎÊýºó£¬ÊÊµ±ÑÓÊ±,ÒÔ±ãÉÏÒ»´Î·¢ËÍµÄÊý¾ÝÕý³£·¢ËÍ³öÈ¥
        #endif
        NVIC_SystemReset();
    }
