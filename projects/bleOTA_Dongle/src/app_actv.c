...
#if (APP_ADV_FIXED_DATA)
    gapm_set_adv_data(
        actv_env.advidx, GAPM_SET_ADV_DATA, APP_ADV_DATA_LEN, (const uint8_t *)APP_ADV_DATA);
#else
...
{
#if (APP_ADV_FIXED_DATA)
    gapm_set_adv_data(actv_env.advidx, GAPM_SET_SCAN_RSP_DATA, APP_SCNRSP_DATA_LEN,
        (const uint8_t *)APP_SCNRSP_DATA);
#else
    uint8_t length;
...
static void app_start_scanning(void)
{
    ...
    scan_param.prop =
        GAPM_SCAN_PROP_PHY_1M_BIT | GAPM_SCAN_PROP_ACTIVE_1M_BIT | GAPM_SCAN_PROP_FILT_TRUNC_BIT;
    ...
    scan_param.scan_param_1m.scan_intv = 8; // GAP_SCAN_FAST_INTV;
    ...
    scan_param.duration = get_scan_duration() * 10; // GAP_TMR_GEN_DISC_SCAN;
    /// Scan period (in unit of 1.28s). Time interval betweem two consequent starts of a scan
    /// duration by the controller. 0 means that the scan procedure is not periodic
    scan_param.period = 0;
...
void app_scan_action(uint8_t actv_op)
{
    switch (actv_op)
    {
        ...
        case ACTV_START:
        {
            ...
            DEBUG("ScanStarting:%d", actv_env.scansta);
...
void app_scan_event(uint8_t gapm_op, uint8_t status)
{
    ...
    switch (gapm_op)
    {
        ...
        case GAPM_STOP_ACTIVITY:
        {
            ...
            DEBUG("ScanStop:%X, %d", status, get_auto_conn());

            if ((status == GAP_ERR_TIMEOUT) && (get_auto_conn()))
            {
                DEBUG("ScanReStart:%d", actv_env.scansta);
                app_scan_action(ACTV_START);
            }
        }
        break;
...
{
    // filter report
    uint8_t adv_info = report->info & GAPM_REPORT_INFO_REPORT_TYPE_MASK;
    if ((adv_info == GAPM_REPORT_TYPE_ADV_LEG) || (adv_info == GAPM_REPORT_TYPE_SCAN_RSP_LEG))
    {
...
void app_init_event(uint8_t gapm_op, uint8_t status)
{
    ...
    switch (gapm_op)
    {
        ...
        case GAPM_STOP_ACTIVITY:
        {
            ...
            if ((status == GAP_ERR_TIMEOUT) && (app_state_get() < APP_CONNECTED))
            {
                init_timer_start();
            }
        }
