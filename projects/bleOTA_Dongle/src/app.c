...
#if !defined(BLE_DRIFT)
    /// Drift default value LMP 5.2
    #define BLE_DRIFT           (500)
#endif
...
// Init BLE and App to startup or Resume BLE to continue if it wakeup from poweroff.
    ...
    if (RSN_IS_BLE_WKUP(rsn))
    {
        ...
        ble_resume();
        rc32k_conf(RCLK_DPLL, RC32K_CAL_CYCLES);
    }
    else
#endif //(BLE_LITELIB)
    {
...
__WEAK void app_conf_fsm(uint8_t evt)
{
    if (evt == BLE_RESET)
    {
        ...
        uint8_t uid[16];
        core_unique_id(sizeof(uid), uid);
        uint32_t crc = crc32(uid, sizeof(uid));
        write32p(ble_dev_addr.addr, crc);
        DEBUG("Local_BT:");
        debugHex(ble_dev_addr.addr, GAP_BD_ADDR_LEN);
        // Set device config
...
__WEAK void app_conn_fsm(uint8_t evt, uint8_t conidx, const void *param)
{
    switch (evt)
    {
        ...
        case BLE_CONNECTED:
        {
            ...
            disc_ota_init(conidx);
        }
        break;

        case BLE_DISCONNECTED:
        {
            if (((struct gapc_disconnect_ind *)param)->reason ==
                LE_ERR_HL2HCI(LL_ERR_CON_TERM_BY_LOCAL_HOST))
            {
                ota_ok_wait_next();
            }
            else
