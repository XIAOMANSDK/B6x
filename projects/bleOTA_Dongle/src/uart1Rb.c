 ...
 *
 * @note When the stop bit time is about 1.5 times the set time, CTMR needs to be used instead of
 *RTO.
 ****************************************************************************************
 */
...
#if (CFG_UART_DMA)
#define CFG_UART_CTMR                                                                              \
    1 // When the stop time is about 1.5 times the set value, CTMR needs to be
      // used
...
void uart1Rb_Init(void)
{
    ...
    uart_fctl(UART1_PORT, (FCR_FIFOEN_BIT | FCR_RXTL_1BYTE), 0,
        /*UART_IR_RXRD_BIT | UART_IR_RXS_BIT |*/ UART_IR_EOB_BIT);
    uartTm_Init();
#else
    ...
    DMA_UARTx_RX_CONF(UART1_DMA_CHAN | DMA_CH_ALT, 1, (uart1RbRx.data + RBUF_HALF_SIZE),
        RBUF_HALF_SIZE, CCM_PING_PONG);
...
#else
    // enable uart IR
    uart_fctl(UART1_PORT, FCR_FIFOEN_BIT | FCR_RXTL_8BYTE, 20 /*bits_rto*/,
        UART_IR_RXRD_BIT | UART_IR_RTO_BIT);
#endif
