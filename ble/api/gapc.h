 ...
 * @file gapc.h
 *
 * @brief Generic Access Profile Controller Messages.
 *
 ****************************************************************************************
 */
...
enum gapc_msg_id
{
    ...
    /// Indicate that a connect_req has been send
    GAPC_CONNECTION_REQ_IND            = MSG_ID(GAPC, 0x01),
    /// Connection has been established, update from v1.3
    GAPC_CONNECTION_IND                = MSG_ID(GAPC, 0x02),
    ...
    GAPC_SMP_CMD                       = MSG_ID(GAPC, 0x12),
    /// Start Bonding command procedure
    GAPC_BOND_CMD                      = GAPC_SMP_CMD, //MSG_ID(GAPC, 0x12),
    /// Start Encryption command procedure
    GAPC_ENCRYPT_CMD                   = GAPC_SMP_CMD, //MSG_ID(GAPC, 0x16),
    /// Start Security Request command procedure
    GAPC_SECURITY_CMD                  = GAPC_SMP_CMD, //MSG_ID(GAPC, 0x1A),
    /// Signature procedure
    GAPC_SIGN_CMD                      = GAPC_SMP_CMD, //MSG_ID(GAPC, 0xF0),
    ...
    /// Set the preferred slave latency (for slave only, with controller)
...
struct gapc_set_phy_cmd
{
    ...
    uint8_t rx_phy;
    /// PHY options (@see enum gap_phy_option)
    uint8_t phy_opt;
};
