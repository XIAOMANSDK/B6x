...
description: |
  分析 B6x 工程 MAP 文件 SRAM 使用情况，检测内存冲突、区域重叠、Stack/Heap 不足。
...
| 检查项 | 条件 | 状态 |
|--------|------|------|
| 区域重叠 | 任何区域地址范围交叉 | 🔴 FAIL |
| 边界溢出 | 超出 0x20008000 | 🔴 FAIL |
| Stack 不足 | < 512 B | 🔴 FAIL |
| Stack 偏小 | < 1 KB | ⚠️ WARNING |
| Heap 不足 | < 1 KB | 🔴 FAIL |
| Heap 偏小 | < 2 KB | ⚠️ WARNING |
| SRAM > 95% | 接近耗尽 | 🔴 FAIL |
| SRAM > 90% | 使用率过高 | ⚠️ WARNING |
...
问题:
1. SRAM 使用率 92% (> 90%)
2. Stack 仅 768 B (< 1 KB)

建议:
1. 减少 ZI 变量，缩小 buffer 大小
2. 在 startup.s 增加 Stack_Size (建议 >= 1024)
```
...
问题:
1. 🔴 区域重叠: Heap (0x20007A00-0x20008000) 与 Stack (0x20007800-0x20008000) 重叠
2. 🔴 SRAM 使用率 97% (> 95%)

必须修复:
1. 检查链接脚本分区配置
2. 减少 Heap 或 User RAM 分配
```
...
请先编译项目:
  ...
  GCC:  cmake -B build && cmake --build build

然后再运行 /b6-sram-analyze
```
